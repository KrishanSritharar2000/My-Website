  ...
  return (
    <Section id="projects">
      ...
      <>
        ...
        >
          {projects.map(
            ({ id, image, title, description, commits, branches, languages, code, live, time, teamSize }) => (
              <Tilt key={id} className="Tilt" scale={0.92}>
                <ProjectCard>
      ...
      <div style={{ display: 'flex', height: '100px', 'justify-content': 'center', 'padding-top': '25px' }}>
        <Button style={{ 'margin-bottom': '0px' }} onClick={() => (window.location = gitHubAddressProjects)}>
          Discover More
        </Button>
      </div>
    </Section>
...
function CardButtons(props) {
  ...
  return (
    <ul style={containerStyle}>
      <HoverEffectButton onClick={() => (window.location = props.code)} left>
        Code
      </HoverEffectButton>
      <HoverEffectButton onClick={() => (window.location = props.liveApp)} right>
        Live App
      </HoverEffectButton>
    </ul>
  );
